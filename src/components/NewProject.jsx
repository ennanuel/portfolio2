...
const NewProject = ({left, number, content}) => {
  ...
  return (
    <div className={`project-container`}>
      <div outertitle={content.name} className={`new-project full-border left-new-project ${isVisible? left? 'animate__animated animate__fadeInLeft': 'animate__animated animate__fadeInRight': 'hidden'}`} ref={nodeRef}>

        <div className={'progress-box'}><div className={`${content.progress === 'finished'? 'finished': 'ongoing'} status`}></div>{content.progress || 'Ongoing'}</div>
